...
#define cu_rc_new(RUDP_RC, ALLOCPTR) ({\
	(RUDP_RC).ptr = cu_allocator_alloc(sizeof(*((RUDP_RC).ptr)), (ALLOCPTR));\
	if ((RUDP_RC).ptr != NULL) {\
		(RUDP_RC).ptr->dt = cu_allocator_alloc(sizeof(*((RUDP_RC).ptr->dt)), (ALLOCPTR));\
		if ((RUDP_RC).ptr->dt != NULL) {\
			(RUDP_RC).ptr->refs = 1;\
			(RUDP_RC).ptr->weakrefs = 0;\
		}\
		else {\
			cu_allocator_free((RUDP_RC).ptr, sizeof(*((RUDP_RC).ptr)), (ALLOCPTR));\
			(RUDP_RC).ptr = NULL;\
		}\
	}\
...
#define cu_rc_isnull(RUDP_RC) ((RUDP_RC).ptr == NULL)

#define cu_rc_copy(RUDP_RC) (++((RUDP_RC).ptr->refs),(RUDP_RC))
#define cu_rc_ptr(RUDP_RC) ((RUDP_RC).ptr->dt)
#define cu_rc_free(RUDP_RC, ALLOCPTR) do {\
	if ((RUDP_RC).ptr->refs == 1) {\
		cu_allocator_free((RUDP_RC).ptr->dt, sizeof(*((RUDP_RC).ptr->dt)), (ALLOCPTR))\
		(RUDP_RC).ptr->dt = NULL;\
		(RUDP_RC).ptr->refs = 0;\
...
#define cu_rc_weak_new(RUDP_RC) (++((RUDP_RC).ptr->weakrefs),(RUDP_RC_TO_WEAKRC_TYPE_INTERNAL_(RUDP_RC)) weak_ptr;}){.weak_ptr = (RUDP_RC).ptr})
#define cu_rc_weak_expired(RUDP_RC_WEAK) ((RUDP_RC_WEAK).ptr->refs == 0)

#define cu_rc_weak_lock(RUDP_RC_WEAK) ((RUDP_RC_WEAK).weak_ptr->refs == 0) ? (RUDP_WEAKRC_TO_RC_TYPE_INTERNAL_(RUDP_RC_WEAK)){.ptr = NULL} : (RUDP_WEAKRC_TO_RC_TYPE_INTERNAL_(RUDP_RC_WEAK)){.ptr = ++((RUDP_RC_WEAK).weak_ptr->refs),(RUDP_RC_WEAK).weak_ptr}

#define cu_rc_weak_free(RUDP_RC_WEAK, ALLOCPTR) do {\
	--((RUDP_RC_WEAK).weak_ptr->weakrefs);\
	if ((RUDP_RC_WEAK).weak_ptr->weakrefs == 0 && (RUDP_RC_WEAK).weak_ptr->refs == 0)\
		cu_allocator_free((RUDP_RC_WEAK).weak_ptr, sizeof(*((RUDP_RC_WEAK).ptr)), (ALLOCPTR));\
...
#define cu_arc_new(RUDP_ARC, ALLOCPTR) ({\
	(RUDP_ARC).ptr = cu_allocator_alloc(sizeof(*((RUDP_ARC).ptr)), (ALLOCPTR));\
	if ((RUDP_ARC).ptr != NULL) {\
		(RUDP_ARC).ptr->dt = cu_allocator_alloc(sizeof(*((RUDP_ARC).ptr->dt)), (ALLOCPTR));\
		if ((RUDP_ARC).ptr->dt != NULL) {\
			atomic_init(&(RUDP_ARC).ptr->refs, 1);\
			atomic_init(&(RUDP_ARC).ptr->weakrefs, 0);\
		}\
		else {\
			cu_allocator_free((RUDP_ARC).ptr, sizeof(*((RUDP_ARC).ptr)), (ALLOCPTR));\
			(RUDP_ARC).ptr = NULL;\
		}\
	}\
...
#define cu_arc_weak_lock(RUDP_ARC_WEAK) ({\
